...
model_save_path = "/home/user/Robotics/SPOTS/models/SVG_tactile_enhanced_split_encoders/saved_models/PRI_single_object_purple_"
...
class ModelTrainer:
    def __init__(self):
        if warm_start:
            print("here")
            self.full_model = torch.load(warm_start_model_path)
    ...
    def train_full_model(self):
        ...
        if warm_start:
            plot_training_loss = list(np.load(warm_start_plot_path + "plot_training_loss.npy"))
            plot_validation_loss = list(np.load(warm_start_plot_path + "plot_validation_loss.npy"))
            best_val_loss = min(plot_validation_loss)
            print("warm starting... Best val loss = {:.4f}".format(best_val_loss))
        else:
            plot_training_loss = []
            ...
            if previous_val_mean_loss < val_mae_losses / index__:
                ...
                if best_val_loss > val_mae_losses / index__:
                    ...
                    torch.save({'encoder_tactile': self.encoder_tactile, 'decoder_tactile': self.decoder_tactile, 'encoder_scene': self.encoder_scene, 'decoder_scene': self.decoder_scene,
                                'frame_predictor': self.frame_predictor, 'posterior': self.posterior, 'prior': self.prior, 'features': features}, model_save_path + "SVG_model")
