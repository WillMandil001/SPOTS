...
model_save_path = "/home/user/Robotics/SPOTS/models/SVG_tactile_enhanced_split_encoders/saved_models/PRI_single_object_purple_AMIR"
warm_start_model_path = "/home/user/Robotics/SPOTS/models/SVG_tactile_enhanced_split_encoders/saved_models/PRI_single_object_purple_model_16_02_2022_16_33/SVG_model"
warm_start_plot_path = "/home/user/Robotics/SPOTS/models/SVG_tactile_enhanced_split_encoders/saved_models/PRI_single_object_purple_model_16_02_2022_16_33/"
...
class ModelTrainer:
    def __init__(self):
        ...
        self.frame_predictor = lstm_models.lstm((g_dim) + z_dim + state_action_size, (g_dim*2), rnn_size, predictor_rnn_layers, batch_size)
        ...
        self.encoder_tactile = model.encoder((g_dim*2), channels)
    ...
    def run(self, scene, touch, actions, test=False):
        ...
        for index, (sample_scene, sample_touch, sample_action) in enumerate(zip(scene[:-1], touch[:-1], actions[1:])):
            ...
            if index > n_past - 1:  # horizon
                ...
                h_tactile_and_scene, skip_tactile_and_scene = self.encoder_tactile(torch.cat([h_scene, x_pred_tactile], 1))
                h_target_tactile_and_scene = self.encoder_tactile(torch.cat([h_target_scene, touch[index + 1]], 1))[0]

                if test:
                    _, mu, logvar = self.posterior(h_target_tactile_and_scene)
                    z_t, mu_p, logvar_p = self.prior(h_tactile_and_scene)
                else:
                    z_t, mu, logvar = self.posterior(h_target_tactile_and_scene)
                    _, mu_p, logvar_p = self.prior(h_tactile_and_scene)

                h_pred = self.frame_predictor(torch.cat([h_tactile_and_scene, z_t, state_action], 1))  # prediction model

                x_pred_tactile = self.decoder_tactile([h_pred, skip_tactile_and_scene])  # prediction model
            ...
            if previous_val_mean_loss < val_mae_losses / index__:
                ...
                if best_val_loss > val_mae_losses / index__:
                    ...
                    torch.save({'encoder_tactile': self.encoder_tactile, 'decoder_tactile': self.decoder_tactile, 'encoder_scene': self.encoder_scene, 'decoder_scene': self.decoder_scene,
                                'frame_predictor': self.frame_predictor, 'posterior': self.posterior, 'prior': self.prior, 'features': features}, model_save_path + "SVTG_SE_AMIR_model")
