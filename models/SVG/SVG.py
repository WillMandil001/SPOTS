...
model_save_path = "/home/willmandil/Robotics/tactile_prediction/tactile_prediction/PRI/SVG/saved_models/PRI_single_object_purple_"
        ...
        for epoch in progress_bar:
            ...
            for index, batch_features in enumerate(self.train_full_loader):
                if batch_features[1].shape[0] == batch_size:
                    ...
                    mae, kld, predictions = self.run(scene=images, actions=action, test=False)
            ...
            with torch.no_grad():
                for index__, batch_features in enumerate(self.valid_full_loader):
                    if batch_features[1].shape[0] == batch_size:
                        images = batch_features[1].permute(1, 0, 4, 3, 2).to(device)
                        action = batch_features[0].squeeze(-1).permute(1, 0, 2).to(device)
                        val_mae, val_kld, predictions = self.run(scene=images, actions=action, test=True)
