...
class BatchGenerator:
    ...
    def load_full_data(self):
        ...
        validation_loader = torch.utils.data.DataLoader(dataset_validate, batch_size=batch_size, shuffle=True, num_workers=1)
        self.data_map = []
...
class ModelTrainer:
    ...
    def train_full_model(self):
        ...
        for stage, epoch_len in zip(training_stages, training_stages_epochs):
            progress_bar = tqdm(range(0, epoch_len), total=(epoch_len*len(self.train_full_loader)))
            gain = 0.0
            for epoch in progress_bar:
                ...
                for index, batch_features in enumerate(self.train_full_loader):
                    ...
                    if batch_features[1].shape[0] == batch_size:
                        ...
                        progress_bar.set_description(stage, " epoch: {}, ".format(epoch) + "MAE: {:.4f}, ".format(float(mae_scene.item())) + "kld: {:.4f}, ".format(float(kld_scene.item())) + "mean MAE: {:.4f}, ".format(mean_mae_scene) + "mean kld: {:.4f}, ".format(mean_kld_scene))
                        progress_bar.update()
                ...
                with torch.no_grad():
                    for index__, batch_features in enumerate(self.valid_full_loader):
                        if batch_features[1].shape[0] == batch_size:
                            ...
                            tactile = batch_features[1].permute(1, 0, 4, 3, 2).to(device)
                            action = batch_features[0].squeeze(-1).permute(1, 0, 2).to(device)
                            val_mae_scene, val_kld_scene, val_mae_tactile, val_kld_tactile, predictions = self.run(scene=images, tactile=tactile, actions=action, test=True, stage=False)
                ...
                if previous_val_mean_loss < val_mae_losses_scene / index__:
                    ...
                    if best_val_loss > val_mae_losses_scene / index__:
                        ...
                        torch.save({"frame_predictor_tactile": self.frame_predictor_tactile, "encoder_tactile": self.encoder_tactile, "decoder_tactile": self.decoder_tactile,
                                    ...
                                    "posterior": self.posterior, "prior": self.prior, 'features': features}, model_save_path + "SVG_SPOTS_BEST")
            ...
            if stage == "scene_only":
                torch.save({"frame_predictor_tactile": self.frame_predictor_tactile, "encoder_tactile": self.encoder_tactile,"decoder_tactile": self.decoder_tactile,
                            "frame_predictor_scene": self.frame_predictor_scene, "encoder_scene": self.encoder_scene, "decoder_scene": self.decoder_scene,
                            "posterior": self.posterior, "prior": self.prior, 'features': features}, model_save_path + "SVG_SPOTS_stage1")
            elif stage == "tactile_loss_plus_scene_fixed":
                torch.save({"frame_predictor_tactile": self.frame_predictor_tactile, "encoder_tactile": self.encoder_tactile,"decoder_tactile": self.decoder_tactile,
                            "frame_predictor_scene": self.frame_predictor_scene, "encoder_scene": self.encoder_scene, "decoder_scene": self.decoder_scene,
                            "posterior": self.posterior, "prior": self.prior, 'features': features}, model_save_path + "SVG_SPOTS_stage2")
            elif stage == "scene_loss_plus_tactile_gradual_increase":
                torch.save({"frame_predictor_tactile": self.frame_predictor_tactile, "encoder_tactile": self.encoder_tactile,"decoder_tactile": self.decoder_tactile,
                            "frame_predictor_scene": self.frame_predictor_scene, "encoder_scene": self.encoder_scene, "decoder_scene": self.decoder_scene,
                            "posterior": self.posterior, "prior": self.prior, 'features': features}, model_save_path + "SVG_SPOTS_stage3")
