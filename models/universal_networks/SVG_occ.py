...
class Model:
    ...
    def save_model(self):
        torch.save({'encoder': self.encoder, 'decoder': self.decoder, 'frame_predictor': self.frame_predictor,
                    'posterior': self.posterior, 'prior': self.prior, 'features': self.features}, self.model_dir + "SVG_occ_model" + self.model_name_save_appendix)
    ...
    def run(self, scene, actions, scene_gt, test=False):
        mae, kld = 0, 0
        outputs = []
