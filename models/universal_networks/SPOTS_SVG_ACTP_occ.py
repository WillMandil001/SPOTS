...
class Model:
    ...
    def run(self, scene, tactile, actions, scene_gt, gain, test=False, stage=False):
        mae_tactile = 0
        kld_tactile = 0
        mae_scene = 0
        kld_scene = 0
        ...
        for index, (sample_scene, sample_tactile, sample_action) in enumerate(zip(scene[:-1], tactile[:-1], actions[1:])):
            ...
            if index > self.n_past - 1:  # horizon
                ...
                h_target_scene      = self.encoder_scene(scene_gt[index + 1])[0]
                ...
                mae_scene += self.mae_criterion_scene(x_pred_scene, scene_gt[index + 1])  # prediction model
    ...
    def save_model(self, stage):
        if stage == "best":
            save_name = "SPOTS_SVG_ACTP_occ_BEST"
        elif stage == "scene_only":
            save_name = "SPOTS_SVG_ACTP_occ_stage1"
        elif stage == "tactile_loss_plus_scene_fixed":
            save_name = "SPOTS_SVG_ACTP_occ_stage2"
        elif stage == "scene_loss_plus_tactile_gradual_increase":
            save_name = "SPOTS_SVG_ACTP_occ_stage3"
