...
model_path      = "/home/user/Robotics/SPOTS/models/SVTG_MMFM/saved_models/PRI_single_object_purple_STVG_MMFM_SE_SOP_TG_model_03_03_2022_16_27/STVG_MMFM_SE_SOP_TG_model"
data_save_path  = "/home/user/Robotics/SPOTS/models/SVTG_MMFM/saved_models/PRI_single_object_purple_STVG_MMFM_SE_SOP_TG_model_03_03_2022_16_27/"
...
(lr, beta1, batch_size, log_dir, model_dir, name, data_root, optimizer, niter, seed, epoch_size,
            ...
            last_frame_skip, epochs, train_percentage, validation_percentage, tactile_gain) = torch.load(model_path)["features"]
...
class ModelTester:
    def __init__(self):
        ...
        (lr, beta1, batch_size, log_dir, model_dir, name, data_root, optimizer, niter, seed, epoch_size,
            image_width, channels, out_channels, dataset, n_past, n_future, n_eval, rnn_size, prior_rnn_layers,
            posterior_rnn_layers, predictor_rnn_layers, z_dim, g_dim, beta, data_threads, num_digits,
            last_frame_skip, epochs, train_percentage, validation_percentage, tactile_gain) = self.full_model["features"]
    ...
    def test_full_model(self):
        ...
        for trial in self.number_of_trials:
            ...
            #             axarr[0].set_title("STVG_MMFM_SE_SOP_TG predictions: t_" + str(i))
        ...
        np.save(data_save_path + "test_no_new_t10_losses_per_trial", np.array(self.total_losses))

        print(self.total_losses)

        final_loss = sum([i[1] for i in self.total_losses]) / len(self.total_losses)

        np.save(data_save_path + "test_no_new_t10_loss", np.array(final_loss))
