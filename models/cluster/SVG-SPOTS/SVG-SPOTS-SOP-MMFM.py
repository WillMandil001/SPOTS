...
model_save_path = "/home/wmandil/Robotics/SPOTS/SPOTS/models/SVG-SPOTS/saved_models/PRI_single_object_purple_SVG_SPOTS_3S_SOP_"
...
class ModelTrainer:
    def __init__(self):
        ...
        self.frame_predictor_scene = lstm_models.lstm(g_dim + z_dim + state_action_size, g_dim, rnn_size, predictor_rnn_layers, batch_size)
        ...
        # TACTILE:
        import models.lstm as lstm_models
        self.frame_predictor_tactile = lstm_models.lstm(g_dim + z_dim + state_action_size, g_dim, rnn_size, predictor_rnn_layers, batch_size)
        ...
        # PRIOR:
        self.MMFM_scene = model.MMFM(g_dim*2, channels)
        ...
        self.MMFM_tactile = model.MMFM(g_dim*2, channels)
    ...
    def run(self, scene, tactile, actions, gain, test=False, stage=False):
        ...
        for index, (sample_scene, sample_tactile, sample_action) in enumerate(zip(scene[:-1], tactile[:-1], actions[1:])):
            ...
            if index > n_past - 1:  # horizon
                ...
                # Tactile Prediction
                h_pred_tactile = self.frame_predictor_tactile(torch.cat([MM_rep_tactile, z_t, state_action], 1))  # prediction model
            ...
            for epoch in range(epoch_len):
                ...
                for index, batch_features in enumerate(self.train_full_loader):
                    ...
                    if batch_features[1].shape[0] == batch_size:
                        ...
                        mae_scene, kld_scene, mae_tactile, kld_tactile, predictions = self.run(scene=images, tactile=tactile, actions=action, gain=gain, test=False, stage=stage)
                ...
                print("TRAINING Stage: ", stage, "mean_mae_scene: ", mean_mae_scene, "mean_kld_scene: ", "mean_kld_scene: ", mean_kld_scene, "mean_mae_tactile: ", mean_mae_tactile, "mean_kld_tactile: ", mean_kld_tactile)
                ...
                with torch.no_grad():
                    for index__, batch_features in enumerate(self.valid_full_loader):
                        if batch_features[1].shape[0] == batch_size:
                            ...
                            tactile = batch_features[1].permute(1, 0, 4, 3, 2).to(device)
                            action = batch_features[0].squeeze(-1).permute(1, 0, 2).to(device)
                            val_mae_scene, val_kld_scene, val_mae_tactile, val_kld_tactile, predictions = self.run(scene=images, tactile=tactile, actions=action, gain=gain, test=True, stage=False)
                            val_mae_losses_scene += val_mae_scene.item()
                            val_mae_losses_tactile += val_mae_tactile.item()

                            if index:
                                val_mean_kld_tactile = val_kld_tactile / index
                                ...
                                val_mean_kld_tactile = 0.0
                                val_mean_mae_tactile = 0.0

                plot_validation_loss.append([val_mean_mae_scene, val_mean_kld_scene, val_mean_mae_tactile, val_mean_kld_tactile])
