...
model_save_path = "/home/user/Robotics/SPOTS/models/SPOTS_SVG_ACTP/saved_models/PRI_single_object_purple_SPOTS_SVG_ACTP_"
...
training_stages_epochs = [50, 25, 50]
lr=0.0001
beta1=0.9
batch_size=32
log_dir='logs/lp'
...
class ModelTrainer:
    def __init__(self):
        ...
        import models.ACTP_model as ACTP_model

        # SCENE:
        import models.lstm as lstm_models
        self.frame_predictor_scene = lstm_models.lstm(g_dim + tactile_size + z_dim + state_action_size, g_dim, rnn_size, predictor_rnn_layers, batch_size)
        self.frame_predictor_scene.apply(utils.init_weights)

        print(g_dim + tactile_size)
        self.MMFM_scene = model.MMFM_scene(g_dim + tactile_size, g_dim + tactile_size, channels)
        self.MMFM_scene.apply(utils.init_weights)
        self.MMFM_scene_optimizer = self.optimizer(self.MMFM_scene.parameters(), lr=lr, betas=(beta1, 0.999))
        ...
        # TACTILE:
        self.frame_predictor_tactile = ACTP_model.ACTP(device=device, input_size=(g_dim + tactile_size), tactile_size=tactile_size)
        self.frame_predictor_tactile.apply(utils.init_weights)

        self.MMFM_tactile = model.MMFM_tactile(g_dim + tactile_size, g_dim + tactile_size, channels)
        self.MMFM_tactile.apply(utils.init_weights)
        self.MMFM_tactile_optimizer = self.optimizer(self.MMFM_tactile.parameters(), lr=lr, betas=(beta1, 0.999))
    ...
    def run(self, scene, tactile, actions, gain, test=False, stage=False):
        ...
        return mae_scene.data.cpu().numpy() / (n_past + n_future), kld_scene.data.cpu().numpy() / (n_future + n_past), \
               mae_tactile.cpu().data.numpy() / (n_past + n_future), torch.stack(outputs)
    ...
    def train_full_model(self):
        ...
        best_val_loss = 100.0
        print(features)
        for stage, epoch_len in zip(training_stages, training_stages_epochs):
            print(stage)
            gain = 0.0
            for epoch in range(epoch_len):
                ...
                for index, batch_features in tqdm(enumerate(self.train_full_loader)):

                    if stage == "scene_loss_plus_tactile_gradual_increase":
                        gain += 0.00001
                        if gain > 0.01:
                            gain = 0.01
                    ...
                    if batch_features[1].shape[0] == batch_size:
                        ...
                        mae_scene, kld_scene, mae_tactile, predictions = self.run(scene=images, tactile=tactile, actions=action, gain=gain, test=False, stage=stage)
                ...
                print("TRAINING Stage: ", stage, "mean_mae_scene: ", mean_mae_scene, "mean_kld_scene: ", "mean_kld_scene: ", mean_kld_scene, "mean_mae_tactile: ", mean_mae_tactile)
                plot_training_loss.append([mean_mae_scene, mean_kld_scene, mean_mae_tactile])
                ...
                val_mae_losses_tactile = 0.0
                val_kld_losses_scene = 0.0
                with torch.no_grad():
                    for index__, batch_features in tqdm(enumerate(self.valid_full_loader)):
                        if batch_features[1].shape[0] == batch_size:
                            images = batch_features[1].permute(1, 0, 4, 3, 2).to(device)
                            tactile = torch.flatten(batch_features[2].permute(1, 0, 2, 3).to(device), start_dim=2)
                            action = batch_features[0].squeeze(-1).permute(1, 0, 2).to(device)
                            val_mae_scene, val_kld_scene, val_mae_tactile, predictions = self.run(scene=images, tactile=tactile, actions=action, gain=gain, test=True, stage=False)
                            val_mae_losses_scene += val_mae_scene.item()
                            val_mae_losses_tactile += val_mae_tactile.item()

                            if index:
                                val_mean_kld_scene = val_kld_scene / index
                                val_mean_mae_scene = val_mae_scene / index
                                val_mean_mae_tactile = val_mae_tactile / index
                            else:
                                val_mean_kld_scene = 0.0
                                val_mean_mae_scene = 0.0
                                val_mean_mae_tactile = 0.0

                plot_validation_loss.append([val_mean_mae_scene, val_mean_kld_scene, val_mean_mae_tactile])
                print("Validation SCENE mae: {:.4f}, Validation TACTILE mae: {:.4f}, ".format((val_mae_losses_scene / index__), (val_mae_losses_tactile / index__)))
                ...
                if previous_val_mean_loss < val_mae_losses_scene / index__:
                    ...
                    if best_val_loss > val_mae_losses_scene / index__:
                        ...
                        torch.save({"frame_predictor_tactile": self.frame_predictor_tactile, "encoder_tactile": self.encoder_tactile, "decoder_tactile": self.decoder_tactile,
                                    ...
                                    "posterior": self.posterior, "prior": self.prior, 'features': features,
                                    "MMFM_scene": self.MMFM_scene, "MMFM_tactile": self.MMFM_tactile}, model_save_path + "SVG_SPOTS_3S_SOP_BEST")

                        best_val_loss = val_mae_losses_scene / index__
                    early_stop_clock = 0
                    previous_val_mean_loss = val_mae_losses_scene / index__

            # save model at the end of each stage:
            if stage == "scene_only":
                torch.save({"frame_predictor_tactile": self.frame_predictor_tactile, "encoder_tactile": self.encoder_tactile,"decoder_tactile": self.decoder_tactile,
                            "frame_predictor_scene": self.frame_predictor_scene, "encoder_scene": self.encoder_scene, "decoder_scene": self.decoder_scene,
                            "posterior": self.posterior, "prior": self.prior, 'features': features,
                            "MMFM_scene": self.MMFM_scene, "MMFM_tactile": self.MMFM_tactile}, model_save_path + "SVG_SPOTS_3S_SOP_stage1")
            elif stage == "tactile_loss_plus_scene_fixed":
                torch.save({"frame_predictor_tactile": self.frame_predictor_tactile, "encoder_tactile": self.encoder_tactile,"decoder_tactile": self.decoder_tactile,
                            "frame_predictor_scene": self.frame_predictor_scene, "encoder_scene": self.encoder_scene, "decoder_scene": self.decoder_scene,
                            "posterior": self.posterior, "prior": self.prior, 'features': features,
                            "MMFM_scene": self.MMFM_scene, "MMFM_tactile": self.MMFM_tactile}, model_save_path + "SVG_SPOTS_3S_SOP_stage2")
            elif stage == "scene_loss_plus_tactile_gradual_increase":
                torch.save({"frame_predictor_tactile": self.frame_predictor_tactile, "encoder_tactile": self.encoder_tactile,"decoder_tactile": self.decoder_tactile,
                            "frame_predictor_scene": self.frame_predictor_scene, "encoder_scene": self.encoder_scene, "decoder_scene": self.decoder_scene,
                            "posterior": self.posterior, "prior": self.prior, 'features': features,
                            "MMFM_scene": self.MMFM_scene, "MMFM_tactile": self.MMFM_tactile}, model_save_path + "SVG_SPOTS_3S_SOP_stage3")
