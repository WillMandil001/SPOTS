...
class Model:
    ...
    def save_model(self):
        torch.save({'encoder': self.encoder, 'decoder': self.decoder, 'frame_predictor': self.frame_predictor,
                    ...
                    self.model_dir + "SVTG_SE_occ_model" + self.model_name_save_appendix)
    ...
    def run(self, scene_and_touch, scene_and_touch_gt, actions, test=False):
        mae, kld = 0, 0
        outputs = []
        ...
        for index, (sample_scene_and_touch, sample_action) in enumerate(zip(scene_and_touch[:-1], actions[1:])):
            ...
            if index > self.n_past - 1:  # horizon
                ...
                h_target = self.encoder(scene_and_touch_gt[index + 1])[0]
                ...
                mae += self.mae_criterion(x_pred, scene_and_touch_gt[index + 1])  # prediction model
                ...
                h_target = self.encoder(scene_and_touch_gt[index + 1])[0]   # should this [0] be here????
